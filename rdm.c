...
int beta, Nmax, nctot, nb, tnb, Spinmax, Blocks,Powns, Powns2, myrank=0 ;
gsl_complex zero, ***Greennew, **Ecluster;
Hamiltonianclass Hc ;

int main(int argc, char *argv[]){
	if(argc != 10){
		printf("Usage :: %s <input.mak> <Hz> <JTD> <SOC> <J> <U_init> <U_final> <DELTA> <save_directory>\n", argv[0]);
		exit(1);
	}
	sectorinfo **ginfo;
	int n, count = 0;
...
void compute_static(typebasis **basis, int count, char *save_directory){
	PNSpara egv[Ni];
	typebasis bin[2];
	int n, degen, gndblock, ic, degeneracy[Ni], mu, nu, i, gnd, phase, *table, index, final_nonzero=0;
	...
	double ***ginformation;
	ginformation = mktritensord(Ni, SDmax, 3);
	...
	sprintf(parardmeval, "%s/result/%c%.3lf/rdmeval.dat", save_directory, control_char, control_para);
	sprintf(parardminfo, "%s/result/%c%.3lf/rdminfo.dat", save_directory, control_char, control_para);
	sprintf(parardmdiag, "%s/result/%c%.3lf/rdmdiag.dat", save_directory, control_char, control_para);
	sprintf(parasusCC, "%s/result/%c%.3lf/susCC.dat", save_directory, control_char, control_para);
	sprintf(parancrdm, "%s/result/%c%.3lf/ncrdm.dat", save_directory, control_char, control_para);
	sprintf(parancrdmevec, "%s/result/%c%.3lf/ncrdmevec.dat", save_directory, control_char, control_para);
	...
	frdmeval = fopen(parardmeval, "w");
	frdminfo = fopen(parardminfo, "w");
	frdmdiag = fopen(parardmdiag, "w");
	fsusCC = fopen(parasusCC, "w");
	Vecop Jop("Jop") ; 
	fncrdm = fopen(parancrdm, "w");
	fncrdmevec = fopen(parancrdmevec, "w");
	...
	for(ic=count; ic<count+1; ic++){
		...
		for(n=0; n<Ni; n++){
			...
			for(mu=0; mu<tNC; mu++) for(nu=0; nu<tNC; nu++)	{
				...
				excitonicT[n][mu][nu] = zero;
				excitonict2g[n][mu][nu] = zero;
			}
			...
			for(degen=0; degen<degeneracy[n]; degen++){
				...
				for(mu=0; mu<Powtnc; mu++) {
					for(nu=0; nu<Powtnb; nu++)  mps[n][nu][mu] = zero;
					for(nu=0; nu<Powtnc; nu++)  rdm[n][mu][nu] = zero;
				}
				...
				for(mu=0; mu<nctot; mu++){
					for(i = gnd; i<gnd+gndblock; i++){	
						doccu[n][mu] += gsl_complex_abs2(ground[i-gnd])* One(basis[i][0], mu) * One(basis[i][1], mu);
						mag[n][mu] += gsl_complex_abs2(ground[i-gnd])* ( (magfactor[2*mu]* (int)One(basis[i][0], mu)) + (magfactor[2*mu+1]* (    int)One(basis[i][1], mu)) );
						filling[n][mu][0] += gsl_complex_abs2(ground[i-gnd])* ((int)One(basis[i][0], mu) );
						filling[n][mu][1] += gsl_complex_abs2(ground[i-gnd])* ((int)One(basis[i][1], mu) );
					}
				}
				for(mu=0; mu<tNC; mu++) for(nu=0; nu<tNC; nu++){
					for(i = gnd; i<gnd+gndblock; i++){	
				...
				for(mu=0; mu<tNC; mu++) for(nu=0; nu<tNC; nu++) excitonict2g[n][mu][nu] = gsl_complex_add( excitonict2g[n][mu][nu], gsl_complex_rect( GSL_REAL(excc.excorderT[mu][nu]), GSL_IMAG(excc.excorderT[mu][nu]) ) ) ;
				excc.dotransform_excorder( Hc.evecEcluster ) ;
				excc.write_excTdeg( pararesult, ic , writemodedegen ) ;
				for(mu=0; mu<tNC; mu++) for(nu=0; nu<tNC; nu++){
					excitonicT[n][mu][nu] = gsl_complex_add( excitonicT[n][mu][nu], gsl_complex_rect( GSL_REAL(excc.excorderT[mu][nu]), GSL_IMAG(excc.excorderT[mu][nu]) ) ) ;
				}
				excc.freegsl() ;
				...
				jsq[n] = gsl_complex_add(	jpjmcalc, jmjpcalc	) ;
				jsq[n] = gsl_complex_mul_real(	jsq[n], 0.5		) ;		gsl_complex jxyjxycalc = gsl_complex_rect( GSL_REAL(jsq[n]), GSL_IMAG(jsq[n]) ) ;
				jsq[n] = gsl_complex_add(	jsq[n], jzjzcalc	) ;
				printf("jxyjxy\t: %19.16f +i %19.16f\n",	GSL_REAL(jxyjxycalc) ,	GSL_IMAG(jxyjxycalc) 	);
				printf(    "jj\t: %19.16f +i %19.16f\n",	GSL_REAL(jsq[n]) , 	GSL_IMAG(jsq[n]) 	);
				...
				write_ABC(	pararesult, ic ,   "JJdeg" , writemodedegen , jsq[n] ,   jzjzcalc ,   jxyjxycalc		) ;
				...
				write_ABC(	pararesult, ic ,   "Lzxydeg" , writemodedegen , lzcalc ,	lxcalc ,	lycalc	) ;
				write_ABC(	pararesult, ic ,   "Szxydeg" , writemodedegen , szcalc ,	sxcalc ,	sycalc	) ;
				...
				double mpssum;
				printf( "startMPS:" ) ;
				mpssum = obtainMPS(	 mps[n], ground, basis, gnd, gndblock )	; printf( "Done.\n" ) ; printf( "mpssum=%19.16f\n", mpssum ) ;
				/*
				printf( "startMPSorb:" ) ;
				...
				*/
				...
				double		jzrdm[Powtnc] ;
				int   		ncrdm[Powtnc] ;
				obtainRDM( rdm[n], mps[n], Powtnb, Powtnc ) ; 
				...
				double	jzsumd4 = 0 ;
				printf("d4_mu_index : " )  ;
				for(mu=0; mu<Powtnc; mu++)  rdmsum += GSL_REAL(rdm[n][mu][mu]) ; 
				for(mu=0; mu<Powtnc; mu++)  jzsum  += GSL_REAL(rdm[n][mu][mu]) * jzrdm[mu]  ; 
				for(mu=0; mu<Powtnc; mu++)  if( ncrdm[mu] == 4 ) { printf("%d ",mu) ; jzsumd4  += GSL_REAL(rdm[n][mu][mu]) * jzrdm[mu]  ; }  printf("\n") ;
				for(mu=0; mu<Powtnc; mu++)  if( ncrdm[mu] == 4 ) rdmsumd4  += GSL_REAL(rdm[n][mu][mu]) ;
				for(mu=0; mu<Powtnc; mu++)  { fprintf( frdmdiag , "%19.16lf\t", GSL_REAL(rdm[n][mu][mu]) ) ; } fprintf( frdmdiag , "\n") ;
				...
				rc.dotransform_LSJd4( rdm[n] ) ;
				rc.dotransform_LSJcubicd4( rdm[n] ) ;
				rc.dotransform_t2gd4( rdm[n] ) ;
				rc.dotransform_Localdiagd4( rdm[n] ) ;
				rc.checkrdmHerimitian( rdm[n] ) ;
				rc.write_jjJd4( pararesult , ic , writemodedegen ) ;
				rc.write_LSJd4( pararesult , ic , writemodedegen ) ;
				rc.write_LSJcubicd4( pararesult , ic , writemodedegen ) ;
				rc.write_t2gd4( pararesult , ic , writemodedegen ) ;
				rc.write_Localdiagd4( pararesult , ic , writemodedegen ) ;
				//rc.obtain_jzmat_LSJd4() ;
				rc.write_target(  pararesult , ic , rdmd4jjJevec, "rdmd4jjJevec" , writemodedegen ) ;
				rc.write_targetd( pararesult , ic , rdmd4jjJeval, "rdmd4jjJeval" , writemodedegen ) ;
				freegscmatrixd(rdmd4jjJevec,DIMd4) ;
				free(rdmd4jjJeval) ;

				eigen_lapack( rdm[n], rdmeval, rdmevec, Powtnc ,0) ; printf("eigen_lapack completed.\n" ) ; //fflush(stdout) ; // Row-wise vectors, i.e. rdmevec[mu][] is the mu-th eigenvectors.
				rc.write_EE( pararesult , ic , rdmeval , writemodedegen ) ;
				...
				for(mu=0; mu<DIMd4; mu++) for(nu=0; nu<DIMd4; nu++) rdmd4[mu][nu] = rdm[n][ rdmvecIndd4[mu] ][ rdmvecIndd4[nu] ] ;printf("rdm is copied to rdmd4.\n" ) ; fflush(stdout) ; 
				for(mu=0; mu<DIMd4; mu++) rdmevald4[mu] = GSL_REAL(rdmd4[mu][mu]) ;
				rc.write_targetd( pararesult , ic , rdmevald4, "rdmdiagd4" , writemodedegen ) ; for(mu=0; mu<DIMd4; mu++) rdmevald4[mu] = 0 ;
				eigen_lapack( rdmd4, rdmevald4, rdmevecd4, DIMd4 ,0) ; printf("eigen_lapack rdmd4 completed.\n" ) ; fflush(stdout) ; // Row-wise vectors, i.e. rdmevec[mu][] is the mu-th eigenvectors.
				rc.write_targetd( pararesult , ic , rdmevald4, "rdmevald4" , writemodedegen ) ;
				rc.write_target(  pararesult , ic , rdmevecd4, "rdmevecd4" , writemodedegen ) ;
				...
				rc.write_target( pararesult , ic , rdmevecd4jjJ, "rdmevecd4jjJ" , writemodedegen ) ;
				...
				rc.write_target( pararesult , ic , rdmevecd4LSJ, "rdmevecd4LSJ" , writemodedegen ) ;
				...
				rc.write_target( pararesult , ic , rdmevecd4t2g, "rdmevecd4t2g" , writemodedegen ) ;
									...
									if( One( bin[nu1%2], nu1/2 ) ) {
										...
										if( Zero( bin[mu1%2], mu1/2 ) ) {  
											...
											jsqrdm = gsl_complex_add( jsqrdm ,
													gsl_complex_mul_real( rdm[n][nu][mu] , phase*jz1*jz2 ) 
													) ;
											//printf("jz(%d,%d,%d,%d,%.2g)",mu1,nu1,mu2,nu2,GSL_IMAG(rdm[n][nu][mu]) );
										}
									}
								}
							}
									...
									if( One( bin[nu1%2], nu1/2 ) ) {
										...
										if( Zero( bin[mu1%2], mu1/2 ) ) {  
											...
											jsqrdm = gsl_complex_add_real( jsqrdm ,
													GSL_REAL(rdm[n][nu][mu]) * phase*jp1*jm2  // 0.5*2Re=1Re
													) ;
											//printf("jpm(%d,%d,%.2g)",mu,nu,rdm[n][nu][mu] );
										}
									}
								}
							}
					...
					for(i = gnd; i<gnd+gndblock; i++){
						...
						if( One( basis[i][mu%2], mu/2) && One( basis[i][nu%2], nu/2 ) && (mu!=nu) ) {
							...
							susCC[mu][nu] = gsl_complex_add_real(
									susCC[mu][nu] , 
									gsl_complex_abs2( mps[n][bstate][CCcstate] )
									//gsl_complex_mul(
									//	mps[n][bstate][cstate] ,
									//	gsl_complex_mul_real( 
									//		gsl_complex_conjugate(mps[n][bstate][CCcstate]) ,
									//		phase )
									//	)
									) ;
						}
			...
			for(mu=0; mu<tNC; mu++) for(nu=0; nu<tNC; nu++)	excitonic[n][mu][nu]	= gsl_complex_div_real( excitonic[n][mu][nu],		degeneracy[n] );
			for(mu=0; mu<tNC; mu++) for(nu=0; nu<tNC; nu++)	excitonicT[n][mu][nu]	= gsl_complex_div_real( excitonicT[n][mu][nu],		degeneracy[n] );
			for(mu=0; mu<tNC; mu++) for(nu=0; nu<tNC; nu++)	excitonict2g[n][mu][nu]	= gsl_complex_div_real( excitonict2g[n][mu][nu],	degeneracy[n] );
			for(mu=0; mu<nctot; mu++){
				mag[n][mu] /= degeneracy[n];
				doccu[n][mu] /= degeneracy[n];
				filling[n][mu][0] /= degeneracy[n];
				filling[n][mu][1] /= degeneracy[n];
				sum[0] += mag[n][mu];
				sum[1] += doccu[n][mu];
			...
			//for(mu=0; mu<nctot; mu++) fprintf(fmag, "%lf\t", mag[n][mu]);
			//for(mu=0; mu<nctot; mu++) fprintf(fdoc, "%lf\t", doccu[n][mu]);
			//for(mu=0; mu<nctot; mu++) fprintf(ffil, "%lf\t%lf\t\t", filling[n][mu][0], filling[n][mu][1]);
			for(mu=0; mu<tNC; mu++) for(nu=0; nu<tNC; nu++){
				fprintf(fext, "%lf\t%lf\t\t", GSL_REAL(excitonic[n][mu][nu]), GSL_IMAG(excitonic[n][mu][nu]));
				if( ic==count && gsl_complex_abs2( excitonic[n][mu][nu] ) > 1e-6 )	final_nonzero = 1;
			}
			write_excT(	pararesult, ic , excitonicT[n] , writemode ) ;
			write_excname(	pararesult, ic , excitonict2g[n] , "excitonict2g" , writemode ) ;
			sprintf(writemode, "a") ;
		}
		//fprintf(fmag, "\n");
		//fprintf(fdoc, "\n");
	...
	if(final_nonzero) for(n=0; n<Ni; n++)	print_gscmatrixd("excitonic",  excitonic[n], tNC, tNC);
	for(n=0; n<Ni; n++)			print_gscmatrixd("excitonicT", excitonicT[n], tNC, tNC);
