...
int main(int argc, char *argv[]){
	...
	for(iu=0; iu<iumax; iu++){
		...
		init_Ecluster( Ecluster, hopmatrix, &quadrature );
		update_chem(Ecluster);
		print_gscmatrixd("Ecluster: after update_chem", Ecluster, NU, NU);
			...
			for(n=0; n<Ni; n++){
				...
				for(i=0; i<degeneracy[n]; i++){
					...
					if( !already_done && myrank == gnode ){
						find_ground(n, save_directory, count, ginfo[n][ argument[n][i] ].ground, basis, ginfo[n], argument[n][i], egv[n], &j, 16, 1);	//machin epsilon ~ 1.11e-16
					}
				}
				for(i=0; i<degeneracy[n]; i++){
					gnode	= argument[n][i]%size;
...
void compute_self(typebasis **basis, int argument[], sectorinfo ginfo[], PNSpara egv, int degeneracy, int count, char *save_directory, gsl_complex ***Self, Glist *table, int n){
	...
	if( PARAMAGNETIC ){
		gsl_complex ***Stemp	= mkgsctritensord(Nmax, tNC, tNC);
		if( Nonzero_SOC ){
			gsl_complex ***Self_t2g	= mkgsctritensord(Nmax, tNC, tNC);
			gsl_complex **transform = mkgscmatrixd(NU, NU);
			init_transform(transform);

			for(i=0; i<Nmax; i++) unitary(Self[i], Self_t2g[i], transform, tNC);

			for(i=0; i<Nmax; i++) for(mu=0; mu<NC; mu++) for(nu=0; nu<NC; nu++) {
				Stemp[i][2*mu][2*nu] = Self_t2g[i][2*mu+1][2*nu+1];
				Stemp[i][2*mu+1][2*nu+1] = Self_t2g[i][2*mu][2*nu];
				Stemp[i][2*mu][2*nu+1] = gsl_complex_mul_real(Self_t2g[i][2*mu+1][2*nu], -1 );
				Stemp[i][2*mu+1][2*nu] = gsl_complex_mul_real(Self_t2g[i][2*mu][2*nu+1], -1 );
			}
			for(i=0; i<Nmax; i++) for(mu=0; mu<tNC; mu++) for(nu=0; nu<tNC; nu++) 
				Self_t2g[i][mu][nu] = gsl_complex_div_real( gsl_complex_add( Self_t2g[i][mu][nu], Stemp[i][mu][nu] ), 2.);
			for(i=0; i<Nmax; i++) iunitary(Self_t2g[i], Self[i], transform, tNC);

			freegsctritensord(Self_t2g, Nmax, tNC);
			freegscmatrixd(transform, NU);
		}
		else{
			for(i=0; i<Nmax; i++) for(mu=0; mu<NC; mu++) for(nu=0; nu<NC; nu++) {
				Stemp[i][2*mu][2*nu] = Self[i][2*mu+1][2*nu+1];
				Stemp[i][2*mu+1][2*nu+1] = Self[i][2*mu][2*nu];
				Stemp[i][2*mu][2*nu+1] = gsl_complex_mul_real(Self[i][2*mu+1][2*nu], -1 );
				Stemp[i][2*mu+1][2*nu] = gsl_complex_mul_real(Self[i][2*mu][2*nu+1], -1 );
			}

			for(i=0; i<Nmax; i++) for(mu=0; mu<tNC; mu++) for(nu=0; nu<tNC; nu++) 
				Self[i][mu][nu] = gsl_complex_div_real( gsl_complex_add( Self[i][mu][nu], Stemp[i][mu][nu] ), 2.);
		}

		freegsctritensord(Stemp, Nmax, tNC);
	}
	///////////////////////
	FILE *ftemp;
	char order[1024];
...
#pragma omp parallel for ordered private(ctemp, i,mu,nu) shared(Greennew, Tbath, Nmax, n) default(none) reduction(+: chisquare)
	for(i=0; i<Nmax; i++){
		ctemp = 0;
		for(mu=0; mu<tNC; mu++) for(nu=0; nu<tNC; nu++){
			ctemp +=
...
#pragma omp parallel for ordered private(i,mu,nu,ctemp) shared(Greennew, Tbath, Nmax, n, nctot, sp) default(none) reduction(+: chisquare)
	for(i=0; i<Nmax; i++){
		ctemp = 0;
		for(mu=0; mu<nctot; mu++) for(nu=0; nu<nctot; nu++){
...
#pragma omp parallel for ordered private(i,mu,nu,ctemp) shared(Greennew, Tbath, Nmax, n) default(none) reduction(+: chisquare)
	for(i=0; i<Nmax; i++){
		ctemp = 0;
		for(mu=0; mu<tNC; mu++) for(nu=0; nu<tNC; nu++){
