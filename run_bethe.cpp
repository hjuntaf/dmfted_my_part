...
void compute_self_bethe(typebasis **basis, int argument[], sectorinfo ginfo[], PNSpara egv, int degeneracy, int count, char *save_directory, gsl_complex ***Self, Glist *table, int n);
int compute_gnew_bethe(gsl_complex ****Self, int count, char *save_directory );
...
gsl_complex ****Self, ****Greennew, zero;
...
int main(int argc, char *argv[]){
	...
	halfbandwidth = read_numberDoubleArr( "inputs/HALFBANDWIDTH", NU );
	char Shead;
	if( fabs(SOC) < 1e-6 ){
		ROTATE = 0;
		Nonzero_SOC = 0;
	...
	for(iu=0; iu<iumax; iu++){
		...
		do{
			...
			omp_set_num_threads( OMPNUMTHREADS );
			compute_gnew_bethe(Self, count, save_directory );
...
void build_argument(int argument[], sectorinfo ginfo[], double gm, int *degeneracy, int forced_gindex, int forced_ndeg){
	...
	*degeneracy = 0;
	...
	if( nstates >= Vmax ){
		if( myrank < 1 ) {
			printf("build_argument: Too many states in %d blocks, nstates %d > %d\n", num_blocks, nstates, Vmax);	fflush(stdout);
			for(i=0; i<num_blocks; i++){
				for(j=0; j<Dmax; j++){
					if( fabs(ginfo[arg_firsts[i]].energy[j] - gm) < Tolerance ){
						printf("argument[%d] = %d\t%.14lf\n", i, arg_firsts[i], ginfo[arg_firsts[i]].energy[j] );
					}
				}
			}
			printf("Done by error.\n"); fflush(stdout);
		}
		MPI_Finalize();
		exit(1);
	}
...
#pragma omp parallel for default(shared) shared(i,Self)
	for(int ii=0; ii<Nmax; ii++) for(int mmu=0; mmu<tNC; mmu++) {
		for(int nnu=0; nnu<tNC; nnu++) Ginverse[ii][mmu][nnu]	= zero;
		Ginverse[ii][mmu][mmu]	= gsl_complex_inverse( Green_total[ii][mmu][mmu] );
	}
...
#pragma omp parallel for default(shared) shared(i,Self)
		for(int ii=0; ii<Nmax; ii++) for(int mmu=0; mmu<tNC; mmu++) {
			for(int nnu=0; nnu<tNC; nnu++) Ginverse[ii][mmu][nnu]	= zero;
			Ginverse[ii][mmu][mmu]	= gsl_complex_inverse( Green_bath[ii][mmu][mmu] );
		}
...
#pragma omp parallel for default(shared) shared(i,Self)
	...
	gsl_complex ***hyb;
	hyb     = mkgsctritensord(Nmax, tNC, tNC);
#pragma omp parallel for default(shared) shared(hyb) collapse(2)
		for(int j=0; j<Nmax; j++) for(int mu2=0; mu2<tNC; mu2++) 	{
			hyb[j][mu2][mu2]		= gsl_complex_mul_real( Green_total[j][mu2][mu2], -halfbandwidth[tNC*n+mu2]*halfbandwidth[tNC*n+mu2]/4. );
			GSL_REAL(hyb[j][mu2][mu2])	-= GSL_REAL(Ecluster[tNC*n+mu2][tNC*n+mu2]);
			GSL_IMAG(hyb[j][mu2][mu2])	+= Matsu[j];
		}
		
#pragma omp parallel for default(shared) shared(Nmax,hyb) collapse(3)
		for(int j=0; j<Nmax; j++) for(int mu2=0; mu2<tNC; mu2++) for(int nu2=0; nu2<tNC; nu2++)	Greennew[n][j][mu2][nu2] = zero;
#pragma omp parallel for default(shared) shared(Nmax,hyb) collapse(2)
		for(int j=0; j<Nmax; j++) for(int mu2=0; mu2<tNC; mu2++) {
			GSL_REAL(Greennew[n][j][mu2][mu2])	= GSL_REAL( hyb[j][mu2][mu2] );
			GSL_IMAG(Greennew[n][j][mu2][mu2])	= GSL_IMAG( hyb[j][mu2][mu2] );
		}
	freegsctritensord(hyb, Nmax, tNC);
...
inline gsl_complex g0iw_bethe( double omega_n , double D , double chem ){
	return gsl_complex_mul_real( 
		gsl_complex_sub( gsl_complex_rect( chem, omega_n ),
				 gsl_complex_mul_real( gsl_complex_sqrt(gsl_complex_rect( -omega_n*omega_n - D*D + chem*chem, 2.*omega_n*chem )), copysign(1.,chem+1e-6) )
		       	),
		2./D/D
		);
}

inline gsl_complex g0iw_bethe_inverse( double omega_n , double D , double chem ){
	return gsl_complex_inverse(  g0iw_bethe( omega_n, D, chem )  );
}

int compute_gnew_bethe(gsl_complex ****Self, int count, char *save_directory ){
	//int Omp_num = omp_get_max_threads();
	int mystart, myend, myblock;
	gsl_complex ***Glocalinverse, ***Glocal, *oneline;
...
#pragma omp parallel shared(oneline,Matsu,Ecluster,Glocal,zero) default(shared)
	for(int i=mystart; i<myend; i++){
		for(int mu=0; mu<NU; mu++) for(int nu=0; nu<NU; nu++){
			Glocal[i][mu][nu]		= zero;
			Glocalinverse[i][mu][nu]	= zero;
		}
		for(int n=0; n<Ni; n++) for(int mu=0; mu<tNC; mu++) {
			gsl_complex omega = gsl_complex_rect(0,0);
			GSL_REAL(omega) =    - GSL_REAL(Ecluster[tNC*n+mu][tNC*n+mu]) - GSL_REAL(Self[n][i][mu][mu]) ;
			GSL_IMAG(omega) = wn - GSL_IMAG(Ecluster[tNC*n+mu][tNC*n+mu]) - GSL_IMAG(Self[n][i][mu][mu]) ;
			Glocalinverse[i][tNC*n+mu][tNC*n+mu] = g0_bethe_complexfreq_inverse( omega, halfbandwidth[tNC*n+mu] );	// G0_inverse
		}
		//inverse_complex_matrix_lapack( Glocalinverse[i], Glocal[i], NU );
		for(int mu=0; mu<NU; mu++) 
			Glocal[i][mu][mu] = gsl_complex_inverse( Glocalinverse[i][mu][mu] );
	}
...
#pragma omp parallel for default(shared) shared(Nmax,Glocal,oneline)
	for(int i=0; i<Nmax; i++) for(int mu=0; mu<NU; mu++) for(int nu=0; nu<NU; nu++)	Glocal[i][mu][nu] = oneline[i*NU*NU+mu*NU+nu];

	gsl_complex ***hyb;
	hyb	= mkgsctritensord(Nmax, tNC, tNC);

	for(int n=0; n<Ni; n++){
#pragma omp parallel for default(shared) shared(Nmax,Glocal,oneline,hyb) collapse(2)
		for(int i=0; i<Nmax; i++) for(int mu=0; mu<tNC; mu++) 	{
			hyb[i][mu][mu]			= gsl_complex_mul_real( Glocal[i][tNC*n+mu][tNC*n+mu], -halfbandwidth[tNC*n+mu]*halfbandwidth[tNC*n+mu]/4. );
			GSL_REAL(hyb[i][mu][mu])	-= GSL_REAL(Ecluster[tNC*n+mu][tNC*n+mu]);
			GSL_IMAG(hyb[i][mu][mu])	+= Matsu[i];
		}
#pragma omp parallel for default(shared) shared(Nmax,Glocal,oneline,hyb) collapse(3)
		for(int i=0; i<Nmax; i++) for(int mu=0; mu<tNC; mu++) for(int nu=0; nu<tNC; nu++)	Greennew[n][i][mu][nu] = zero;
#pragma omp parallel for default(shared) shared(Nmax,Glocal,oneline,hyb) collapse(2)
		for(int i=0; i<Nmax; i++) for(int mu=0; mu<tNC; mu++) {
			GSL_REAL(Greennew[n][i][mu][mu])	= GSL_REAL( hyb[i][mu][mu] );
			GSL_IMAG(Greennew[n][i][mu][mu])	= GSL_IMAG( hyb[i][mu][mu] );
		}
	}
	freegsctritensord(hyb, Nmax, tNC);
	free(offset);
	...
	if( myrank == 0 ){
		...
		//}
#pragma omp parallel for default(shared) shared(Nmax,Glocal,oneline,hyb) collapse(2)
		for(int i=0; i<Nmax; i++) for(int mu=0; mu<NU; mu++) {
			for(int nu=0; nu<NU; nu++) Glocalinverse[i][mu][nu]	= zero;
			Glocalinverse[i][mu][mu]	= gsl_complex_inverse( Glocal[i][mu][mu] );
		}
